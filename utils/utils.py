...
class ErcTextDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, DATASET='MELD', SPLIT='train', speaker_mode='upper',
        ...
        self.SEED = SEED
        if 'roberta' in model_checkpoint or 'bart' in model_checkpoint:
            self.sep_token = '</s></s>'
        else:
            self.sep_token = '[SEP]'
            ...
            for dialogue in raw_data:
                ...
                for idx, ue in enumerate(ues):
                    ...
                    input_ = {'input_ids': input_ids,
                              'attention_mask': attention_mask, 'label': label, 'current_ids': current_ids, 'current_masks': current_masks}
...
if __name__ == '__main__':
    ds_train = ErcTextDataset(DATASET='IEMOCAP', SPLIT='train',
